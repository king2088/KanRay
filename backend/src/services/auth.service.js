...
async function userWithRoles(userId) {
  ...
  return { ...publicUser(row), roles: await rolesOf(userId), permissions: await permissionsOf(userId) };
}

async function register({ email, password, name = '', roleCode }) {
...
async function login(email, password) {
  const mail = String(email || '').trim().toLowerCase();
  const row = await db.prepare('SELECT * FROM users WHERE email = ?').get(mail);
  ...
  const issued = await issueTokens(row.id);
  return { ...issued, user: await userWithRoles(row.id) };
}
...
async function refresh(refreshToken) {
  ...
  const row = await db.prepare('SELECT * FROM refresh_tokens WHERE user_id = ? AND token_hash = ?').get(userId, hash);
  if (!row || row.revoked_at || new Date(row.expires_at) < new Date()) throw new HttpError(401, '刷新令牌无效');
  await db.prepare('UPDATE refresh_tokens SET revoked_at = ? WHERE id = ?').run(new Date().toISOString(), row.id);
  const user = await db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  if (!user || !user.is_active) throw new HttpError(401, '账号不可用');
  return { ...await issueTokens(userId), user: await userWithRoles(userId) };
}

async function logout(accessPayload) {
...
async function changePassword(userId, oldPassword, newPassword) {
  ...
  await logout({ sub: userId });
  return true;
}

async function updateProfile(userId, { name }) {
  await db.prepare("UPDATE users SET name = ?, updated_at = datetime('now') WHERE id = ?").run(String(name || '').slice(0, 50), userId);
