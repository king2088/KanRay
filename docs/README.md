...
| 角色 | 先读 | 再读 |
|------|------|------|
| 业务用户（上传数据 / 建图表 / 搭看板） | [快速开始](快速开始.md) | [用户手册](01-用户手册.md) |
...
| 运维（Docker 部署 / 存储切换 / 密钥管理） | [部署运维手册](05-部署运维手册.md) | [常见问题 FAQ](06-常见问题FAQ.md) |

## 文档清单

| 文档 | 内容 | 篇幅 |
|------|------|------|
| [需求清单-第一阶段](需求清单-第一阶段.md) | 第一阶段需求：数据集/图表/看板、DataProvider 与 Excel 分析 | ~ |
| [需求清单-第二阶段](需求清单-第二阶段.md) | 第二阶段需求：多用户 + RBAC + 多数据源 + 开放 API | ~ |
...
| [05-部署运维手册](05-部署运维手册.md) | Docker 一键部署、环境变量全表、存储后端切换、密钥轮换、已知限制 | ~5 千字 |
| [06-常见问题FAQ](06-常见问题FAQ.md) | 按场景问答：登录 / 数据源 / 同步 / 分享 / 开放API 常见问题 | ~3 千字 |

## 编写约定

- 基准：以当前代码（`feature/m2-datasources` 分支合并后）为准
- 默认账号：`admin@kanban.local / admin123`（生产环境务必修改，见部署手册）
