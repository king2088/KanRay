...
- [x] **Step 1: 测试基座 `backend/test/helpers/db.js`**
...
- [x] **Step 2: 写失败测试 `backend/test/task1-seed.test.js`**
...
- [x] **Step 1: 写失败测试 `backend/test/task2-jwt.test.js`**
...
- [x] **Step 3: 实现 `backend/src/utils/jwt.js`**
...
- [x] **Step 1: 写失败测试 `backend/test/task3-auth.test.js`**
...
- [x] **Step 3: 实现 `backend/src/services/auth.service.js`**
...
- [x] **Step 1: 写失败测试 `backend/test/task4-audit.test.js`**
...
- [x] **Step 2: 运行确认失败** → `cd backend && node --test test/task4-audit.test.js`（FAIL）

- [x] **Step 3: 实现 `backend/src/services/audit.service.js`（若 Task 3 已建则跳过本步）**
...
- [x] **Step 1: 写失败测试 `backend/test/task5-rbac.test.js`**
...
- [x] **Step 2: 运行确认失败** → `cd backend && node --test test/task5-rbac.test.js`（FAIL）

- [x] **Step 3: 实现 `backend/src/services/rbac.service.js`**
...
- [x] **Step 1: 写失败测试 `backend/test/task6-admin.test.js`**
...
- [x] **Step 2: 运行确认失败** → `cd backend && node --test test/task6-admin.test.js`（FAIL）

- [x] **Step 3: 实现 `backend/src/routes/admin.routes.js`**
...
- [x] **Step 2: 创建 `front-end/src/stores/auth.js`**
...
- [x] **Step 3: `front-end/src/api/index.js` 追加**
...
- [x] **Step 4: `front-end/src/api/http.js` 改造**
...
- [x] **Step 6: 路由守卫 `front-end/src/router/index.js`**
...
- [x] **Step 1: 创建 `front-end/src/components/layout/ProfileDialog.vue`**
...
- [x] **Step 2: 创建 `front-end/src/components/layout/UserMenu.vue`**
...
- [x] **Step 1: 用户管理 `front-end/src/views/admin/UserAdmin.vue`**
...
- [x] **Step 2: 角色管理 `front-end/src/views/admin/RoleAdmin.vue`**
...
- [x] **Step 3: 审计 `front-end/src/views/admin/AuditView.vue`**
...
- [x] **Step 3: 越权回归（CDP e2e-rbac.cjs）**
...
- [x] 密码：所有注册/建号/重置密码都走 bcrypt（`hashSync(...,10)`），无明文落库。
- [x] token：`JWT_SECRET` 环境变量可注入（config.auth.jwtSecret），dev 默认值仅本地。
- [x] refresh：数据库存 SHA-256 哈希，单次使用轮换。
- [x] SQL 注入：新增查询全用 prepared statement（better-sqlite3 `?`）。
- [x] 越权：`/api/admin/*` 双层校验（`user:read` + 具体动作），数据出口（query/data/rows）均过 `assertResource`。
- [x] `.gitignore` 检查：`backend/data/`（含 kanban.db）与 `backend/uploads/` 不入库。

- [x] **Step 5: 更新 README + 需求清单标注 + 提交**
