...
- 测试运行：`cd backend && node --test test/`（`test/**/*.test.js` 自动发现）。Node `node:test` 每个测试文件独立进程，`process.env.DB_PATH` 须在 `require('../src/...')` 之前设置（见 Task 1 的 helper 约定，所有测试文件必须首行设置 `process.env.DB_PATH`）。
...
顺带全量：`cd backend && node --test test/` 应全部通过。
